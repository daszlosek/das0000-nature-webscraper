...
nsmb %>% html_node(paste0("time")) %>% html_text2()
...
subjournal_html_parser <- function(subjournal = "nsmb",volume = 1,issue = 1,child_node = 1) {
          ...
          date <- tmp_html %>% html_node(paste0("time")) %>% html_text2()
          
          bind_cols(
                    # subjournal = subjournal, # removed since it is in mapply
                    # volume = volume,
                    # issue = issue,
...
all_comb2 <- expand.grid(issue = 1:2, child_node = 1:2, subjournal = c("nsmb"),volume = 1:2, stringsAsFactors=FALSE)

all_comb2
vector_output <- do.call(mapply, c(

        list(FUN=nature_html_parser, MoreArgs = NULL, SIMPLIFY = TRUE, USE.NAMES = TRUE),all_comb2))

if (is.matrix(vector_output)) vector_output <- t(vector_output)

vector_output

vector_output %>% data.frame() %>% unlist %>% data.frame() %>%
...
parse_title <- vector_output %>% unlist %>% data.frame() %>% t()
parse_title[2]
strsplit(vector_output,2)


test_df <- stringr::str_split(parse_title," ")

test_df
parse_title

test <- cbind(all_comb2, parse_title)

test

typeof(test)

article_combination_list <- cmapply(issue = 1:3, child_node = 1:2, subjournal = c("nsmb"),volume = 1:2, FUN=nature_html_parser)
...
cmapply <- function(FUN, ..., MoreArgs = NULL, SIMPLIFY = TRUE, USE.NAMES = TRUE) {
    ...
    # transpose matrix
    if (is.matrix(vector_output)) vector_output <- t(vector_output) 
    # combined output to initial combination
    # removed in my version since I already do it above
    #cbind(all_comb, vector_output)
    #vector_output
    parse_title <- vector_output %>% unlist
...
#                     dplyr::select(issue_1,child_node_2,subjournal_3,volume_4,parsed_title)
