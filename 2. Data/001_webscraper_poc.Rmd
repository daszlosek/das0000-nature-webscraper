...
site_index <- read_html("https://www.nature.com/siteindex") 

# remove some crap
site_index_clean1 <- site_index %>% html_nodes("a") %>% html_attr("href") %>% data.frame() %>%
               filter(str_detect(.,"#") != T )

# remove a little more 
site_index_clean2 <- site_index %>% html_nodes("a") %>% html_attr("href") %>% data.frame() %>%
               filter(str_detect(.,"#|https") != T )


# remove most (I think? all)
site_index_clean3 <- site_index %>% html_nodes("a") %>% html_attr("href") %>% data.frame() %>%
