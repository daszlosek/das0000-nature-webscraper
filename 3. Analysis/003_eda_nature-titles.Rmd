...
cleaned_df <- read_rds(here::here("3. Analysis",paste0(params$nb_id,"_cleaned_df.rds")))
...
ggplot(raw_data, aes(x = ngram, group = journal_title, color = journal_title)) +
       geom_density() +
       scale_y_continuous("Density", expand = c(0,0)) +
...
```{r}
ggplot(raw_data, aes(x = journal_title, y = string_count, 
...
cleaned_df %>% ungroup() %>% group_by(journal_title) %>% summarise(median = ngram)
